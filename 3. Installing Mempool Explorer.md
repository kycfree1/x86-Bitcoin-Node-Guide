...
Hosting your own instance locally is preferable to entering personal transaction details into websites you don’t control.
...
```
```bash
torsocks curl -fsSL https://download.docker.com/linux/debian/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg
```
```bash
...
sudo apt update
```
Install Docker networking utilities.

```bash
sudo apt install uidmap dbus-user-session slirp4netns docker-compose-plugin -y
```
Create an unprivileged Mempool user.

```bash
sudo adduser --disabled-login --gecos "" --shell /usr/bin/bash unprivmempool
```
Access the new user.

```bash
sudo su - unprivmempool
```
Install rootless Docker as your unprivileged user.

```bash
torsocks curl -fsSL https://get.docker.com/rootless | sh
```

...
After fully synchronizing Bitcoin Core, installing an Electrum indexer is the next step, which ensures compatibility with all Electrum-based Bitcoin wallets.
...
cd ~/downloads
```
In a browser, visit [Fulcrum’s Github page](https://github.com/cculianu/Fulcrum/releases) and copy the link for the latest “_x86_64-linux.tar.gz_” file. At the time of writing, the most recent version is 1.9.6.

Download Fulcrum.

```bash
wget https://github.com/cculianu/Fulcrum/releases/download/v1.9.6/Fulcrum-1.9.6-x86_64-linux.tar.gz
```
Download the releases "asc" file.

```bash
wget https://github.com/cculianu/Fulcrum/releases/download/v1.9.6/Fulcrum-1.9.6-sha256sums.txt.asc
```
Download the release's "sha256sum" file.

```bash
wget https://github.com/cculianu/Fulcrum/releases/download/v1.9.6/Fulcrum-1.9.6-sha256sums.txt
```
Verify Fulcrum's checksum.

```bash
sha256sum -c --ignore-missing Fulcrum-*-sha256sums.txt
```
...
```bash
gpg --verify Fulcrum-*-sha256sums.txt.asc
```
...
```bash
rm Fulcrum-*-sha256sums.txt && rm Fulcrum-*-sha256sums.txt.asc
```
...
```bash
wget https://github.com/cculianu/Fulcrum/releases/download/v0.0.0/Fulcrum-0.0.0-x86_64-linux.tar.gz
```
...
```bash
wget https://github.com/cculianu/Fulcrum/releases/download/v0.0.0/Fulcrum-0.0.0-x86_64-linux.tar.gz.asc
```
```bash
wget https://github.com/cculianu/Fulcrum/releases/download/v0.0.0/Fulcrum-0.0.0-x86_64-linux.tar.gz.sha256sum
```

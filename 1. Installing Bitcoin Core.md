...
**Prerequisites.**
 ...
 - [ ] Minimum 1 Terabyte NVMe/SSD storage (2 Terrabyte recommended).
...
```
Paste the following lines, then save and exit the file with "control+x," confirm with "y," and then "enter."
...
Copy the URL for the latest "x86_64-linux-gnu.tar.gz" package and download with "wget."
...
```
This will output a series of signature checks for each public key that signed the checksums. The keys previously imported to your keyring should show a "gpg: Good signature" message.
...
chmod +x rpcauth.py
```
Run the following command, replacing "passwordhere" with a strong Bitcoin RPC password. Avoid the use of special characters.
...
```
Before continuing, wait until the initial block download completes. IBD speeds are limited by Tor; however, the privacy benefits of avoiding clearnet are substantial.
...
Bitcoin Core has now been successfully installed, synchronized, and configured with Tor.
...
cd ~/downloads
```
Copy the URL for the latest "x86_64-linux-gnu.tar.gz" package and download it with "wget." The following command can be edited to include the required version number.

...
If logged in as "root" with 0 created users, first ensure sudo is installed.
...
```bash
sudo apt install curl gpg unzip -y
```
...
Enable package sources accessible over HTTPS.
...
```
Paste the following lines, then save and exit the file.
